...
app.use(function(req, res, next) {
	...
	if (env.swiftcashd && env.swiftcashd.rpc) {
		req.session.host = env.swiftcashd.host;
		req.session.port = env.swiftcashd.port;
		req.session.username = env.swiftcashd.rpc.username;

		global.client = new bitcoin.Client({
			host: env.swiftcashd.host,
			port: env.swiftcashd.port,
			user: env.swiftcashd.rpc.username,
			pass: env.swiftcashd.rpc.password,
			timeout: 5000
		});
	}
